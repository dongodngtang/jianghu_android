...
public class LinkifyHelper {
     ...
     */
    public static final boolean addLinks(Spannable text, SmartTextCallback mSmartTextCallback, int mask) {
        ...
        for (LinkSpec link : links) {
            applyLink(link.url, link.start, link.end, text, mSmartTextCallback);
        }
    ...
    private static final void applyLink(String url, int start, int end, Spannable text, SmartTextCallback mSmartTextCallback) {
        SmartClickSpan span = new SmartClickSpan(url);
        span.setSmartTextCallback(mSmartTextCallback);
    ...
    // 正则表达式，提取我们所有匹配的内容；
    private static final void gatherTelLinks(ArrayList<LinkSpec> links, Spannable sp) {

        Pattern pattern = Pattern
                .compile("\\d{3}-\\d{8}|\\d{4}-\\d{7}|\\d{11}");
        Matcher matcher = pattern.matcher(sp.toString());
        int start = 0;
        //遍历取出字符串中所有的符合条件的；
        while (matcher.find(start)) {
            start = matcher.end();
            LinkSpec spec = new LinkSpec();
            spec.url = "tel:" + matcher.group();
            spec.start = matcher.start();
            spec.end = matcher.end();
            links.add(spec);
            if (start >= sp.toString().length()) {
                break;
            }
        }
