...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
        ...
        if (timestamp != null) {
            if (position == 0) {
                timestamp.setText(DateUtils.getChatStrDate(message.getMsgTime()));
        ...
        if (message.direct() == Direct.SEND) {
            itemClickListener.setUserAvatar(context, itemClickListener.getMeChatUserAvatar(), userAvatarView);
        } else {
            itemClickListener.setUserAvatar(context, itemClickListener.getToChatUserAvatar(), userAvatarView);
            EaseUserUtils.setUserNick(itemClickListener.getToChatNickname(), usernickView);
        }
...
//                            }
    ...
    protected void updateView() {
        Log.d("EaseUI sendMessage",message.status()+"");
        activity.runOnUiThread(new Runnable() {
            public void run() {
//                if (message.status() == EMMessage.Status.FAIL) {
//
//                    if (message.getError() == EMError.MESSAGE_INCLUDE_ILLEGAL_CONTENT) {
//                        CommonUtils.showSnack(activity, activity.getString(R.string.send_fail) + activity.getString(R.string.error_send_invalid_content));
//                    } else if (message.getError() == EMError.GROUP_NOT_JOINED) {
//                        CommonUtils.showSnack(activity, activity.getString(R.string.send_fail) + activity.getString(R.string.error_send_not_in_the_group));
//                    } else {
//                        CommonUtils.showSnack(activity, activity.getString(R.string.send_fail) + activity.getString(R.string.connect_failuer_toast));
//                    }
//                }
