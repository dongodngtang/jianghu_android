 ...
 * 日期：16-1-12.
 */
public class PageLive extends PageBase<NodeTimeline.ItemsBean> {

    private Context mContext;
    private String user_id;

    public PageLive(Context context, String user_id) {

        items = new ArrayList<>();
        adapter = new UserLiveAdapter(context, items);
        mRequest = new UserTimelineGet(user_id);
        mRequest.setCancelSign(context);
        mContext = context;
    ...
    @Override
    public UserTimelineGet getRequestParam() {
        if (getItems().size() > 0 &&
        ...
        return (UserTimelineGet) mRequest;
    }
